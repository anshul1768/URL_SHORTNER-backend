...
const loginUser=async(req,res)=>{
    ...
    const isCorrect=user.isPasswordCorrect(user.password);
    if(!isCorrect){
    ...
    const safeUser=await User.findById(user._id).select("-password -_id")
    res.status(201).cookie("accessToken",accessToken,options).json(new ApiResponse(201,safeUser,"user logged in successfully."))
}
